                                            ...
                                            <div class="table-responsive">
                                                <table class="table table-striped custom-table table-hover">
                                                    ...
                                                    <tbody>
                                                        <?php
                                                        $counter = 1;
                                                        $query = mysqli_query($db, "select * from result where uniq_id='$uniqid'");
                                                        if (mysqli_num_rows($query) == 0) {
                                                            echo "<tr style='text-align:center;'><td colspan=14 style='font-size:25px;'>No Quizzes Attempted Yett!!!</td></tr>";
                                                        } else {
                                                            while ($row = mysqli_fetch_array($query)) {
                                                                extract($row);
                                                                $query1 = mysqli_query($db, "select q.quizname,p.first_name,p.last_name from quiz q join appuser p where q.quiz_id='$quiz_id' and p.uniq_id='$prof_id'");
