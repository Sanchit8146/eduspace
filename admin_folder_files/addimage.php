...
<body style="user-select:none;" spellcheck="false"
                                            ...
                                            <form id="imageform" method="POST">
                                                <div class="card-head row">
                                                    <header class="col-md-6">
                                                        <label>Input the image for gallery</label>
                                                        <input type="file" class="form-control" name="snap" id="file"
                                                            placeholder="Add Image" accept="image/*" required>
                                                        <small class="text-muted">*File size should be less than
                                                            4MB</small>
                                                    </header>
                                                    <header class="col-md-4">
                                                        <input type="submit"
                                                            style="display:block;margin:20px auto 10px auto ;"
                                                            value="Add" class="btn btn-circle btn-success btn-lg">
                                                    </header>
    ...
    $('#file').on('change', function() {
        var s = (this.files[0].size / 1024 / 1024).toFixed(2);
        if (s >= 4) {
            alert('This file size is greater than 4MB. Upload another image or use image compression tool!!');
            $('#file').val("");
        }
    });
    </script>
        ...
        $('#imageform').on('submit', function(e) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    message_status.fadeIn(10000);
                    message_status.text(data);
                    message_status.fadeOut(10000);
                    $('#file').val('');
                }
            })
        })
